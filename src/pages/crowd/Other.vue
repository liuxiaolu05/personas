    ...
    export default {
        ...
        data: function(){
            ...
            return {
                ...
                // 计算覆盖人数需要的字段
                calFields: ["scenesType", "catPrefer", "brandPrefer", "regionCode", "firstBranch", "secondBranch", "store", "customerTransaction", "vipValue", "loyalty", "lost", "gender", "age"],
                calValidateFields: ["group", "regionCode", "firstBranch", "secondBranch", "store"],
                categoryShow:false,
                editData: {
                    scenesType: "2",
                    //其它场景
                    ...
                    brandPrefer:[],//品牌偏好
                    brandName:[],//品牌名称
                    customerTransaction:["1","2","3","4","5"],//平均客单价
                    vipValue:["1","2","3"],//会员价值
                    loyalty:["1","2","3"],//忠诚度
                    lost:["1","2","3"],//流失等级
                    gender:["1","0"],//性别
                ...
                ruleValidate:{
                    /**多个字段组合校验，全部为空的时候弹框提示
                     * 品类偏好、品牌偏好、平均客单价、会员价值、忠诚度、流失等级、性别、年龄
                     * */
                    group: [
                        {
                            trigger: 'change',
                            validator: function(rule, value, callback){
                                var fileds = ["catPrefer", "brandPrefer", "customerTransaction", "vipValue", "loyalty", "lost", "gender", "age"],
                                        data = _vm.editData, count = 0;
                                for(var i in fileds){
                                    var filed = fileds[i];
                                    data[filed].length && count++;
                                }
                                if(!count){
                                    _vm.$Modal.info({
                                        title: "提示信息",
                                        content: '<p style="margin-bottom: 5px;">以下选项至少一项不能为空：</p>' +
                                        '<p>品类偏好、品牌偏好、平均客单价、会员价值、忠诚度、流失等级、性别、年龄</p>'
                                    });
                                    callback(" ");
                                }else{
                                    callback();
        ...
        mounted:function(){
            this.$nextTick(function() {
                $('.vip-value>.ivu-FormItem-label').html('<span>会员价值</span><div  class="ivu-tooltip"><div class="ivu-tooltip-rel"><i class="ivu-icon  ivu-icon-ios-help-outline"></i></div> <div class="ivu-tooltip-popper" x-placement="top"><div class="ivu-tooltip-content"><div class="ivu-tooltip-arrow"></div> <div class="ivu-tooltip-inner">根据购买频次，购买金额、最近一次购买时间进行建模分析，将会员价值分为三个等级，购买频次、购买金额越高，最后一次购买时间距今越短，会员价值越高</div></div></div></div>');
                $('.loyalty>.ivu-FormItem-label').html('<span>忠诚度</span><div  class="ivu-tooltip"><div class="ivu-tooltip-rel"><i class="ivu-icon  ivu-icon-ios-help-outline"></i></div> <div class="ivu-tooltip-popper" x-placement="top"><div class="ivu-tooltip-content"><div class="ivu-tooltip-arrow"></div> <div class="ivu-tooltip-inner">主要依据购买频次，购买总次数因素进行建模分析，购买次数与频次越高，会员忠诚度越高</div></div></div></div>');
                $('.lost>.ivu-FormItem-label').html('<span>流失等级</span><div  class="ivu-tooltip"><div class="ivu-tooltip-rel"><i class="ivu-icon  ivu-icon-ios-help-outline"></i></div> <div class="ivu-tooltip-popper" x-placement="top"><div class="ivu-tooltip-content"><div class="ivu-tooltip-arrow"></div> <div class="ivu-tooltip-inner">主要依据购买频次和最近一次购买时间因素进行建模分析，最近一次购买时间距今时间越长、购买频次越低流失等级相对较高&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div></div></div></div>');

            })
        },
        methods:{
            handleClose:function(e,n){
                this.editData.catPrefer.splice(n,1)
            },
            selectCategory:function(e){
                var _vm=this;
                if(this.editData.catPrefer.indexOf(e.target.innerHTML)==-1){
                    _vm.editData.catPrefer.push(e.target.innerText)
                }
            },
        }
    }
