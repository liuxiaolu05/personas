<template>
    <div class="sale-situation" id="brand">
        <div class="sale-top box-bot title-box">
            ...
            <div class="box-content sale-form">
                ...
                <div class="form-condition self-btn-ghost date-component">
                        ...
                        <div class="form-group monthly-report" v-show="monthlyHigh">
                        ...
                        <div class="form-group monthly-report" v-show="yearlyHigh">
                            <date-picker type="year"  placement="bottom-start"  :value="date.year" placeholder="选择日期" style="width: 200px" @on-change="yearTime"></date-picker>
                        </div>
                    </div>
            </div>
        </div>
        <div class="sale-data title-box">
            ...
            <div class="box-content">
                ...
                <div class="table box-bot">
                    ...
                    <!--<i-table border :columns="columns" :data="dataTable" @on-sort-change="sort"></i-table>-->
    ...
    export default{
        ...
        methods: {
            ...
            getEchartData:function(){
                ...
                self.params = {
                    ...
                    startTime: self.date.startTime,
                    endTime: self.date.endTime,
                    currPage: 1,
                    pageSize: 10
                };
                axios.get(getUrl("brandChart"),{
                    params:self.params
                })
                        .then(function(res){
                            self.spinShow=false;
            ...
            getTableData: function () {
                ...
                self.params = {
                    ...
                    ordertype: self.order.orderType== 'normal' ? 'desc' : self.order.orderType,
                    currPage: self.page.currPage,
                    pageSize: self.page.pageSize
                };
                self.noDataInfo = '数据加载中...';
                axios.get(getUrl("brandTable"),{
                    params:self.params,
                    loading: true,
                    unload: true
                })
                        .then(function(res){
                            if(res.CommunityAnalysis && res.CommunityAnalysis.tableList.length > 0){
                                ...
                                if(self.initialization){
                                    $('#brand .ivu-table-header thead .ivu-table-cell').each(function(){
                                        var $span = $(this).find('span:eq(0)');
                                        $span.replaceWith('<i>'+$span.text()+'</i>');
                                    });
                                    $('#brand .sort-name .ivu-table-sort').find('.ivu-icon-arrow-down-b').addClass('on');
            ...
            sort: function (column) {
                ...
                $('#brand .ivu-table-sort').find('.ivu-icon').removeClass('on');
        ...
        mounted: function() {
            ...
            for(var cur in _this.dataName){
                ...
                switch(cur){
                    case '订单数':
                        comUtils.help(itemObj,'订单数','所选时间范围内，该小区（楼盘）某个品牌产生的零售单数量');
                        break;
                    case '销售金额':
                        comUtils.help(itemObj,'销售金额','所选时间范围内，该小区（楼盘）某个品牌的销售金额总和');
                        break;
                    case '退货数':
                        comUtils.help(itemObj,'退货数','所选时间范围内，该小区（楼盘）某个品牌产生的退货零售单数量');
                        break;
                    case '退货金额':
                        comUtils.help(itemObj,'退货金额','所选时间范围内，该小区（楼盘）某个品牌退货零售单中销售金额之和');
                        break;
                    case '平均消费价格':
                        comUtils.help(itemObj,'平均消费价格','所选时间范围内，该小区（楼盘）某个品牌销售金额/商品数');
                        break;
