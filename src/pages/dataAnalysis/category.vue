<template>
    <div class="sale-situation" id="category">
            ...
            <div class="box-content sale-form">
                ...
                <div class="date-component">
                    <div class="daily form-condition self-btn-ghost  " v-show="dailyHigh">
                        ...
                        <div class="form-group"><!--:value="defaultDay" -->
                    ...
                    <div class="form-condition" v-show="weeklyHigh">
                        <div class="form-group week-select i-pull-clear">
        ...
        <div class="sale-data title-box">
            ...
            <div class="box-content">
                ...
                <div class="table box-bot">
                    <Table border stripe :columns="columns" :no-data-text="noDataInfo" :data="dataTable" @on-sort-change="sort" ></Table>
                    <div class="i-pull-right page">
                        <!-- <Page :total="totalPage" show-elevator show-sizer :placement="direction"  :page-size="page" show-total @on-change="change"></Page>-->
    ...
    export default{
        ...
        methods: {
                        ...
                        .then(function (res) {
                            if (res.week && res.week.length > 0) {
                                ...
                                weekData.forEach(function (item, index) {
                                    obj = {};
                                    obj.value = item;
                                    obj.label = item.replace('-', '.');
                                    option.push(obj);
                                });
                                self.week.optionData = option;
                                self.week.option = option[0].value;
                            } else {
                                self.week.option = '';
            ...
            getTableData: function () {
                ...
                self.params = {
                    ...
                    ordertype: this.order.orderType == 'normal' ? 'desc' : this.order.orderType,
                    currPage: this.page.currPage,
                    pageSize: this.page.pageSize
                };
                self.noDataInfo = '';
                axios.get(getUrl("classTable"), {
                    params: self.params,
                    loading: true,
                    unload: true
                })
                        .then(function (res) {
                            if (res.CommunityAnalysis && res.CommunityAnalysis.tableList.length > 0) {
                                ...
                                self.page.totalPage = totalData.totalCount;
                                tableData = totalData.tableList;
                                self.requestTableData(tableData);
                                if (self.initialization) {
                                    $('#category .ivu-table-header thead .ivu-table-cell').each(function(){
                                        var $span = $(this).find('span:eq(0)');
                                        $span.replaceWith('<i>'+$span.text()+'</i>');
                                    });
                                    $('#category .sort-name .ivu-table-sort').find('.ivu-icon-arrow-down-b').addClass('on');
            ...
            getChartData: function () {
                ...
                axios.get(getUrl("classChart"), {
                    params: self.params
                })
                        .then(function (res) {
                            if (res.CommunityAnalysis && res.CommunityAnalysis.chart && res.CommunityAnalysis.chart.gome_sale) {
                                self.noData = true;
                                totalData = res.CommunityAnalysis;
            ...
            changeData: function (e) {
                if (this.firstLoad) {
                    this.date.startTime = e; //请求周报数据赋值导致初始化时加载此方法
                    this.getAllData();
                } else {
                    this.firstLoad = true
                }
            ...
            sort: function (column) {
                ...
                $('#category .ivu-table-sort').find('.ivu-icon').removeClass('on');
        ...
        mounted: function () {
            ...
            for (var cur in _this.dataName) {
                ...
                itemObj.className = cur == '订单数' ? 'sort-name ' + _this.dataName[cur] : _this.dataName[cur];
                /* if (cur == '订单数') {
                 itemObj.className = 'sort-name ' + _this.dataName[cur];
                 }*/
                if (cur != '品类') {
                ...
                switch(cur){
                    case '订单数':
                        comUtils.help(itemObj,'订单数','所选时间范围内，该小区（楼盘）某个品类产生的零售单数量');
                        break;
                    case '销售金额':
                        comUtils.help(itemObj,'销售金额','所选时间范围内，该小区（楼盘）某个品类的销售金额总和');
                        break;
                    case '退货数':
                        comUtils.help(itemObj,'退货数','所选时间范围内，该小区（楼盘）某个品类产生的退货零售单数量');
                        break;
                    case '退货金额':
                        comUtils.help(itemObj,'退货金额','所选时间范围内，该小区（楼盘）某个品类退货零售单中销售金额之和');
                        break;
                    case '平均消费价格':
                        comUtils.help(itemObj,'平均消费价格','所选时间范围内，该小区（楼盘）某个品类销售金额/商品数');
                        break;
